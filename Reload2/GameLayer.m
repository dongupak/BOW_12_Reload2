...
-(id) init
{
	if( (self=[super init])) {
        ...
        //ptReload의 줄어드는 방향, 줄어드는 정도 등 여러가지 옵션을 설정합니다.
        //(여기서는 오른쪽부터 수평으로만 줄어들게 설정했습니다.)
        ptReload.barChangeRate = ccp(1, 0);
        ptReload.midpoint = ccp(0, .5);
        //ptReload의 비율을 0으로 잡습니다.
        //(처음에는 총알이 최대치로 장전되어있으므로 비율을 0으로 하여 안 보이게 해 놓아야 하기 때문입니다.)
        ptReload.percentage=0;
        //총의 종류를 나타내는 스프라이트 spGun의 앞쪽에 위치해야 하므로 z값을 더 크게 잡았습니다.
        //(spGun의 z값 : 6, ptReload의 z값 : 9)
...
-(void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    //총알이 0이 된다면
        ...
        //2초 동안 0%가 될 때 까지 줄어 든다.
        id actionReload = [CCProgressTo actionWithDuration:10
                                                   percent:0];
        //위 액션이 끝나면 다음 함수 수행
